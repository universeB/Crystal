...
import crystal.voter.Votes;
import crystal.voter.VotingChoice;
...
public class CalculateProjectTask extends SwingWorker<Void, Result> {
	 ...
	 */
	@Override
	protected synchronized Void doInBackground() throws IOException {
		...
		Vector<VotingChoice> options = new Vector<VotingChoice>();
		//If there is a conflict detected, where the merge sign rising, start voting for the decision
		if(localStateResult.getAction().equals("git merge"))
			startVoting(options);
		    ...
		    if (mine != null) {
		        for (DataSource source : _prefs.getDataSources()) {
		            ...
		            if (commonParentSource != null) {
		                ...
		                // calculate the When
		                ourRelationship.setWhen(mine.getWhen(yours, parent, ourRelationship)); // calculate the Consequences
	...
	private void startVoting(Vector<VotingChoice> options) {
		Votes.startVotingProcess(options);
	}
